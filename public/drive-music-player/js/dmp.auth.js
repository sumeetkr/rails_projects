...
dmp.auth.initAuth = function(callback) {
  ...
  if (!dmp.auth.hasGoneTroughAuth()) {
    ...
    var userId = userId ? userId : dmp.url.getHashParameter("userId");
    ...
    if (!stateUrlParam && (fileIdsHashParam || folderIdHashParam)) {
      var stateObj = {};
...
dmp.auth.periodicCheckAuth = function(callback) {
  ...
  if (dmp.auth.hasGoneTroughAuth()) {
    ...
    // Start the autorefresh chron job.
    window.setInterval(dmp.auth.autoRefreshAuth, 1000 * 3600 / 2);
...
dmp.auth.autoRefreshAuth = function(callback) {
  gapi.auth.authorize({
    ...
    if (authResult.error) {
      ...
      // Rebuild the picker so it uses the new access token.
      dmp.ui.buildPicker();
    }
