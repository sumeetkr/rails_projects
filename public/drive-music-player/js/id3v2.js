/*
 * Copyright (c) 2009 Opera Software ASA, Ant—nio Afonso (antonio.afonso@opera.com)
...
(function(ns) {
    ...
    function readFrames(offset, end, data, id3header)
    {
        ...
        while( offset < end ) {
            ...
            switch( major ) {
                case 2:
                var frameID = frameData.getString(3, frameDataOffset);
                var frameSize = frameData.getUint8(frameDataOffset+3) * Math.pow(2, 16) + frameData.getUint16(frameDataOffset+4, false);
                var frameHeaderSize = 6;
                break;

                case 3:
                var frameID = frameData.getString(4, frameDataOffset);
                var frameSize = frameData.getUint32(frameDataOffset + 4, false);
            ...
            if (frameID == "APIC" || frameID == "PIC") {
              ...
              var parsedData = window.btoa(binary);
              var desc = mimeType;
              ...
              var parsedData = frameData.getString(frameSize, frameDataOffset, unicode).replace(/\0/g, "");
            }
